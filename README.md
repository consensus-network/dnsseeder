...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/consensus-network/dnsseeder)

DNSSeeder exposes a list of known peers to any new peer joining the
Consensusnetwork via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the
`consensusd` node specified with the `-s` flag and listen for `addr`
...
file, so on subsequent start ups the `consensusd` node specified with `-s`
...
* Launch a `consensusd` node for the DNSSeeder to connect to
...
```bash
git clone https://github.com/consensus-network/dnsseeder ${GOPATH}/src/github.com/consensus-network/dnsseeder
cd ${GOPATH}/src/github.com/consensus-network/dnsseeder
go install .
```
...
`consensusd` instances, set the following DNS records:
